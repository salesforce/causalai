...
class DifferenceTransform(BaseTransform):
    ...
    def __init__(self, order: int=1):
        """
        :param order: the interval at which two time steps are chosen for taking a difference
        :type order: int
        """
    ...
    def transform(self, *data: List[ndarray]) -> Union[List[ndarray],ndarray]:
        ...
        transformed_data = []
        for i in range(len(data)):
            new_data = np.stack([data[i][j] - data[i][j - self.order] for j in range(self.order, len(data[i]))])
            transformed_data.append(new_data)
        return transformed_data if len(transformed_data)>1 else transformed_data[0]
