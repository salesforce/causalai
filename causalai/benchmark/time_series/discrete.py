'''
This is the benchmarking module for discrete time_series data. This module supports methods that evaluates causal discovery algorithms against various challenges, 
...
import tqdm
from .base import base_synthetic_time_series_benchmark, BenchmarkDiscreteTimeSeriesBase

class BenchmarkDiscreteTimeSeries(BenchmarkDiscreteTimeSeriesBase):
    '''
    Discrete time_series data benchmarking module. This class inherits the methods and 
    variables from BenchmarkTimeSeriesBase and BenchmarkDiscreteTimeSeriesBase, and defines 
    ...
    def __init__(self, algo_dict:Dict=None, kargs_dict:Dict=None, num_exp:int=20, custom_metric_dict:Optional[Dict]={}, **kargs):
        ...
        '''
        BenchmarkDiscreteTimeSeriesBase.__init__(self, algo_dict=algo_dict, num_exp=num_exp,
    ...
    def benchmark_sample_complexity(self, T_list:List[int] = [100, 500,1000,5000], num_vars:int=20, graph_density:float=0.1, data_max_lag:int=3,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
    ...
    def benchmark_graph_density(self, graph_density_list:List[float] = [0.05, 0.1, 0.2, 0.5], num_vars:int=20, T:int=1000, data_max_lag:int=3,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
    ...
    def benchmark_data_max_lag(self, data_max_lag_list:List[int] = [1,5,10], num_vars:int=20, graph_density:float=0.1, T:int=1000,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
        Graph density: Benchmark algorithms on synthetic data with different number of samples. 
        The synthetic data for any variable is generated using a structural equation model (SEM) of the form:
