'''
This is the benchmarking module for continuous time_series data. This module supports methods that evaluates causal discovery algorithms against various challenges, 
...
from .base import base_synthetic_time_series_benchmark, BenchmarkContinuousTimeSeriesBase

class BenchmarkContinuousTimeSeries(BenchmarkContinuousTimeSeriesBase):
    '''
    Continuous time_series data benchmarking module. This class inherits the methods and 
    variables from BenchmarkTimeSeriesBase and BenchmarkContinuousTimeSeriesBase, and defines 
    ...
    def __init__(self, algo_dict:Dict=None, kargs_dict:Dict=None, num_exp:int=20, custom_metric_dict:Optional[Dict]={}, **kargs):
        ...
        :param algo_dict: A Python dictionary where keys are names of causal discovery algorithms, and 
            ...
            must be inherited from the `BaseTimeSeriesAlgoFull` class that can be found in causalai.models.time_series.base.
            Crucially, this class constructor must take a `TimeSeriesData` object (found in causalai.data.time_series) as input, 
        ...
        '''
        BenchmarkContinuousTimeSeriesBase.__init__(self, algo_dict=algo_dict, num_exp=num_exp,
                                     kargs_dict=kargs_dict, custom_metric_dict=custom_metric_dict, **kargs)

    def benchmark_noise_type(self, noise_fn_types:List[Callable]=[np.random.randn, np.random.rand], 
                            noise_fn_names:List[str]=['Gaussian', 'Uniform'],
                            num_vars:int=5, graph_density:float=0.1, T:int=1000, data_max_lag:int=3, fn:Callable = lambda x:x, coef:float=0.1):
        '''
        Continuous noise type: Benchmark algorithms on synthetic data generated using different noise functions. 
        ...
        :param noise_fn_types: A list of callable functions, which when called, returns a scalar value. E.g. numpy.random.randn.
        :type noise_fn_types: List[Callable]
        :param noise_fn_names: A list of noise function names (string) corresponding to the list of noise functions
            provided in noise_fn_types.
        :type noise_fn_names: List[str]
        ...
        :type T: int
        :param data_max_lag: max lag value to be used to generate the causal graph. 
            This value must be an integer greater than 0. For instance, a max lag of 2 implies that the parents 
            of a node (the the generated causal graph) cannot be more than 2 time steps behind the time step of that node.
        :type data_max_lag: int
        ...
        :type coef: float
        '''
        assert type(noise_fn_types)==list and type(noise_fn_names)==list,\
                            f'noise_fn_types and noise_fn_names must be lists, but found types {type(noise_fn_types)} and '\
                            f'{type(noise_fn_names)}.'
        assert len(noise_fn_types)==len(noise_fn_names), f'noise_fn_types and noise_fn_names must be lists of the same length, '\
                                                f'but found lengths {len(noise_fn_types)} and {len(noise_fn_names)}. '\
                                                f'noise_fn_types is a list of callable functions. noise_fn_names is a list '\
                                                f'of the corresponding names.'

        self.variant_values = noise_fn_names
        self.variant_name = 'Noise Type'
        all_results = []
        for noise_fn in noise_fn_types:
            noise_fn_list = [noise_fn]*num_vars
            result_list = base_synthetic_time_series_benchmark(self.algo_dict, 
                                              self.kargs_dict, noise_fn_list, num_vars=num_vars,
                                              graph_density=graph_density, T=T, data_max_lag=data_max_lag, num_exp=self.num_exp,
                                              fn = fn,
                                              coef=coef,
                                              discrete=False,
    ...
    def benchmark_snr(self, snr_list:List[float]=[0.01, 0.1, 1.], num_vars:int=5, graph_density:float=0.1, T:int=1000, data_max_lag:int=3,\
                           fn:Callable = lambda x:x, noise_fn:Callable=np.random.randn):
        '''
        Signal to noise ratio: Benchmark algorithms on synthetic data generated with different signal to noise ratios. 
        The synthetic data for any variable is generated using a structural equation model (SEM) of the form:

        child = sum_i coef* parent_i + noise

        We vary SNR by varying the value of coef above. Since the scale of noise is fixed, the ratio of coef to the scale
        of noise acts as SNR.
        
        :param snr_list: It contains list of graph denity values to be used to generate the causal graph. 
            Each value must be in (0,1].
        :type snr_list: List[float]
        :param num_vars: Integer value specifying the number of variables in the generated data.
        :type num_vars: int
        :param graph_density: Float value in (0,1] specifying the density of the causal graph. The value is used 
            as the probability with which an edge between 2 nodes exists during the causal graph generation process.
        ...
        self.variant_values = snr_list
        self.variant_name = 'SNR'
        for coef in snr_list:
            noise_fn_list = [noise_fn]*num_vars
            result_list = base_synthetic_time_series_benchmark(self.algo_dict, 
                                              self.kargs_dict, noise_fn_list, num_vars=num_vars,
                                              graph_density=graph_density, T=T, data_max_lag=data_max_lag, num_exp=self.num_exp,
    ...
    def benchmark_variable_complexity(self, num_vars_list:List[int] = [2,10,20,40], graph_density: float=0.1, T:int=1000, data_max_lag:int=3,\
    ...
    def benchmark_sample_complexity(self, T_list:List[int] = [100, 500,1000,5000], num_vars:int=5, graph_density:float=0.1, data_max_lag:int=3,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
    ...
    def benchmark_graph_density(self, graph_density_list:List[float] = [0.05, 0.1, 0.2, 0.5], num_vars:int=5, T:int=1000, data_max_lag:int=3,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
    ...
    def benchmark_data_max_lag(self, data_max_lag_list:List[int] = [1,5,10], num_vars:int=5, graph_density:float=0.1, T:int=1000,\
                           fn:Callable = lambda x:x, coef:float=0.1, noise_fn:Callable=np.random.randn):
        '''
        Graph density: Benchmark algorithms on synthetic data with different number of samples. 
        The synthetic data for any variable is generated using a structural equation model (SEM) of the form:
        ...
        :param data_max_lag_list: It contains list of max lag values to be used to generate the causal graph. 
            Each value must be an integer greater than 0. For instance, a max lag of 2 implies that the parents 
            of a node (the the generated causal graph) cannot be more than 2 time steps behind the time step of that node.
        :type data_max_lag_list: List[int]
        :param num_vars: Integer value specifying the number of variables in the generated data.
        :type num_vars: int
        :param graph_density: Float value in (0,1] specifying the density of the causal graph. The value is used 
            as the probability with which an edge between 2 nodes exists during the causal graph generation process.
